...
#define SERVERADDRESS "192.168.101.201"
...
int main(int argc, char ** argv) {
	...
	while (true) {
		...
		while (enet_host_service(client, &event, 0) >= 1) {
			...
			{
				unsigned char* host = (unsigned char*)&event.peer->address.host;
				printf("connect to server %d.%d.%d.%d:%u.\n", host[0], host[1], host[2], host[3], event.peer->address.port);
				hasconnected = true;
			}
				break;
